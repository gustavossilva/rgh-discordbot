            ...
            try {
                ...
                if (typeof serverRequestedStatus !== "undefined") {
                    if (serverRequestedStatus) {
                        message.channel.send(`Server ${parsedServerString} is currently `+SubHealerEmoji+" ONLINE! Have fun o/");
                    } else {
                        message.channel.send(`Server ${parsedServerString} is currently `+SubDPSEmoji+" OFFLINE! Go watch some youtube or drink beer");
                    }
                } else {
                    const randomString = Math.floor((Math.random() * funnyArray.length));
                    message.channel.send("I can't find the server you typed, try something like"+"\n\n"+"-serverstatus Exodus"+"\n\n"+"And here is a joke or a fact: "+funnyArray?.[randomString]);
                }
            } catch (e) {
                console.error(e);
                message.channel.send("Something is wrong with me, please contact Dizzy or E'vhano :smiling_face_with_tear:");
            }
        }
    }
}